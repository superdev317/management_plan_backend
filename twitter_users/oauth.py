...
class Token(object):
    ...
    def _get_token(self, url, token=None, method='POST', **parameters):
        ...
        response, content = client.request(url,
            method  = method,
            headers    = urllib3.request.urlencode(parameters)
        )
    ...
    def _get_access_token(self, url, token=None, method='POST', **parameters):
        client            = oauth2.Client(self.consumer, token)

        response, content = client.request(url,
            method  = method,
            body    = urllib3.request.urlencode(parameters)
        )
        if response['status'] != '200':
            return None;
...
class AccessToken(Token):
    def __init__(self, consumer, oauth_token, oauth_verifier):
        ...
        token_content = self._get_access_token(ACCESS_TOKEN_URL, oauth_token=oauth_token, oauth_verifier=oauth_verifier)
        test = str(token_content, "utf-8").strip()

        self.params   = parse_qs(test)
